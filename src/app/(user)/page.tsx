...
function ProductCardSkeleton() {
  return (
    <div className="bg-white border border-warm-beige overflow-hidden rounded-lg animate-pulse">
      <div className="aspect-square bg-cream/50" />
      <div className="p-3 sm:p-4 space-y-3">
        <div className="h-2 bg-cream/50 rounded w-1/2" />
        <div className="h-4 bg-cream/50 rounded w-3/4" />
        <div className="h-5 bg-cream/50 rounded w-1/3" />
        <div className="flex gap-2">
          <div className="flex-1 h-8 bg-cream/50 rounded" />
          <div className="flex-1 h-8 bg-cream/50 rounded" />
        </div>
      </div>
    </div>
...
function CategoryCardSkeleton() {
  return (
    <div className="relative aspect-[3/4] bg-neutral-200/50 rounded-2xl animate-pulse" />
  );
}
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 md:space-y-16">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left">
        <div className="space-y-2 sm:space-y-3 w-full">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50/50 border border-amber-200/50">
            <div className="w-3 h-3 bg-amber-200 rounded-full animate-pulse" />
            <div className="h-3 w-20 bg-amber-200 rounded animate-pulse" />
          </div>
          <div className="h-8 bg-cream/50 rounded w-48 animate-pulse" />
          <div className="w-12 sm:w-16 h-0.5 bg-amber-200 animate-pulse" />
        </div>
        <div className="h-4 bg-cream/50 rounded w-24 animate-pulse" />
      </div>
    ...
    <section className="bg-white border-t border-b border-neutral-200/80 py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12 md:space-y-16">
        <div className="text-center space-y-2 sm:space-y-3">
          ...
          <div className="h-8 bg-cream/50 rounded w-48 mx-auto animate-pulse" />
          <div className="h-4 bg-cream/50 rounded w-64 mx-auto animate-pulse" />
        </div>
...
export default function HomePage() {
  ...
  // Load featured products when visible
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && loadingFeatured) {
  ...
  // Load recent products when visible
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && loadingRecent) {
            loadRecentProducts();
          }
        });
      },
  ...
  // Load categories when visible
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && loadingCategories) {
            loadCategories();
          }
        });
      },
  ...
  const loadRecentProducts = async () => {
    try {
      const productsData =
        products.length > 0 ? products : await productApi.getAll();
  ...
  return (
    <div className="bg-[#FAF8F5] min-h-screen space-y-12 sm:space-y-16 md:space-y-20 lg:space-y-24 pb-12 sm:pb-16 md:pb-20 lg:pb-24">
      ...
      {/* Featured Products - Lazy Load */}
      <div id="featured-section">
        <Suspense fallback={<SectionSkeleton title="Featured Collection" />}>
          {loadingFeatured ? (
            <SectionSkeleton title="Featured Collection" />
          ) : (
      ...
      {/* Recent Products - Lazy Load */}
      <div id="recent-section">
        <Suspense fallback={<SectionSkeleton title="Newest Additions" />}>
          {loadingRecent ? (
            <SectionSkeleton title="Newest Additions" />
          ) : (
      ...
      {/* Why Choose Us - Static (no loading needed) */}
