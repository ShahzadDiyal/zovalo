// src/app/(user)/shop/ShopClient.tsx
"use client";
...
export function ShopClient({
  ...
  const availableSeaters = useMemo(() => {
    ...
    products.forEach((product) => {
      if (product.seaterCount && Array.isArray(product.seaterCount)) {
        product.seaterCount.forEach((seater) => {
          if (seater && typeof seater === "string") {
            seaters.add(seater.trim());
          }
        });
      }
    });
  ...
  // Update URL when search changes (for real-time)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <LoadingSpinner />
        <span className="text-xs font-bold uppercase tracking-[0.25em] text-amber-600 animate-pulse">
          Curating Collection...
        </span>
      </div>
    );
            ...
            {currentProducts.length === 0 ? (
              <div className="py-16 bg-white border border-dashed border-neutral-200/80 rounded-2xl text-center">
                <EmptyState
                  ...
                  description="We couldn't find any products matching your specific refine parameters. Try adjusting your search query or removing filters."
