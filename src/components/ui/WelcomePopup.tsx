...
import { X, Gift, Sparkles, ArrowRight, Star, ChevronRight } from 'lucide-react';
...
export default function WelcomePopup({ products }: WelcomePopupProps) {
  ...
  const diningProduct = products.find(p => 
    p.title?.toLowerCase().includes('dining') || p.category === 'Dining Tables'
  );
  const sofaProduct = products.find(p => 
    p.title?.toLowerCase().includes('sofa') || p.category === 'Sofa Sets'
  );
  const bedProduct = products.find(p => 
    p.title?.toLowerCase().includes('bed') || p.category === 'Beds'
  );

  const featuredProducts = [sofaProduct, diningProduct, bedProduct].filter(Boolean);
        ...
        <div className="p-6 sm:p-8 md:p-10">
          ...
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-500 text-sm sm:text-base max-w-md mx-auto">
              Discover luxury furniture crafted for the modern home. 
              Enjoy exclusive welcome offers on your first purchase.
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-gold/20 via-gold/10 to-gold/20 border border-gold/30 rounded-xl p-4 sm:p-6 mb-10 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-gold mb-2">🎁 Special Offer</p>
                  ...
                  <div className="bg-cream border border-warm-beige rounded-lg overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-1">
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-sm font-light text-near-black">
                        {product?.price ? formatCurrency(product.price) : 'Call for price'}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-mint-50 flex items-center justify-center">
                  <svg className="w-3 h-3 text-mint-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <span className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider">Free UK Delivery</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-mint-50 flex items-center justify-center">
                  <svg className="w-3 h-3 text-mint-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider">Cash on Delivery</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-full bg-mint-50 flex items-center justify-center">
                  <svg className="w-3 h-3 text-mint-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className="text-[9px] sm:text-[10px] text-gray-500 uppercase tracking-wider">Secure Shopping</span>
              </div>
            </div>
          </div>
        </div>
